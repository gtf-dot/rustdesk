...
def get_features(args):
    ...
    if not windows and not osx and not os.environ.get('VCPKG_ROOT'):
...
def build_flutter_deb(version, features):
    ...
    if ships_so:
        ...
        os.rename('../horusrd-%s.deb' % version, f'../{DRM_PACKAGE_NAME}-{version}.deb')
    os.chdir("..")
...
def assert_staged_binary_is_drm():
    ...
    """
    binaries = [p for p in glob.glob('tmpdeb/usr/share/horusrd/lib/librustdesk.so')
                + glob.glob('tmpdeb/usr/share/horusrd/horusrd') if os.path.isfile(p)]
...
def build_deb_from_folder(version, binary_folder, want_drm=False):
    ...
    bundled_glob = glob.glob('tmpdeb/usr/share/horusrd/libdrmtap.so.0.*')
    ...
    if want_drm:
        ...
        if bundle_carries_so:
            ...
            system2(f'rm -f "{so}"')
            system2('rm -f tmpdeb/usr/share/horusrd/libdrmtap.so tmpdeb/usr/share/horusrd/libdrmtap.so.0')
        else:
...
def build_flutter_dmg(version, features):
    ...
    # the .app name follows the Xcode product name, which may differ from RustDesk
    apps = [a for a in glob.glob(f'{flutter_build_dir}*.app') if not a.endswith('.dSYM')]
    if len(apps) != 1:
        raise RuntimeError(f'expected exactly one .app in {flutter_build_dir}, found {apps}')
    system2(f'cp -rf ../target/release/service "{apps[0]}/Contents/MacOS/"')
    '''
    system2(
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/HorusRD.exe')
    os.chdir('../..')
    ...
    os.rename('./rustdesk_portable.exe', f'./HorusRD-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/HorusRD-{version}-install.exe')
